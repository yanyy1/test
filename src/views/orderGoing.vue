<template>
    <section v-if="hasInfo">
        ...
        <carItem  :financeId="financeIdArray[index]" :key="index" :infoObj="item" v-on:target="target(index)" :downPayment="downPay[index]" :mouthPayment="monthPay[index]"></carItem>
        </div>
    </section>
</template>
<script>
...
export default {
    ...
    methods: {
        initOrder: function () {
            ...
            this.$http.get(BASE_URL+'app/v1/order/getOrderByPhoneNumberAndStatus?phoneNumber=' + username + '&status=1', { headers: { access_token: token } }).then((res) => {
                //根据电话号码获取订单信息
        ...
        target: function (index) {
            ...
            this.$router.push({ path: "carParticulars", query: {carId: carId,orderId: orderId } });
        }
    },
    computed: {
